...
router.post('/', async (req, res) => {
  const { part_code, part_name, brand, description, revision } = req.body;
  if (!part_code || !part_name) {
    return res.status(400).json({ error: 'part_code and part_name are required' });
  }
  try {
    const result = await pool.query(
      `INSERT INTO main_part_master (part_code, part_name, brand, description, revision)
       VALUES ($1, $2, $3, $4, COALESCE($5, 'A')) RETURNING *`,
      [part_code, part_name, brand || null, description || null, revision]
    );
...
router.put('/:id', async (req, res) => {
  const { part_code, part_name, brand, description, revision, status } = req.body;
  try {
    const result = await pool.query(
      `UPDATE main_part_master
       ...
       WHERE main_part_id = $7 RETURNING *`,
      [part_code, part_name, brand, description, revision, status, req.params.id]
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Main part not found' });
    }
